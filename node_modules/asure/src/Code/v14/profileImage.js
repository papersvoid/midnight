...
const moment = require("moment")
  ...
  registerFont(`${process.cwd()}/node_modules/asure/src/Fonts/Helvetica.ttf`, {
    family: `Helvetica Normal`,
  });
  registerFont(`${process.cwd()}/node_modules/asure/src/Fonts/Helvetica-Bold.ttf`, {
    family: `Helvetica Bold`,
  });

async function profileImage(userParam){

    if(discordJS == '^12.5.3') throw new Error('Asure | Discord.js V12 is not supported')

    const userTypes = ["User", "ClientUser", "GuildMember"]

    let user = userParam

    if(!user) throw new Error('Asure | You must add a parameter of type User | GuildMember')
    if(!userTypes.includes((user).constructor.name)) throw new Error('Asure | The selected parameter is not of type User | GuildMember')
    if(user.constructor.name == "GuildMember") user = user.user;
    ...
    const base = await read(`${process.cwd()}/node_modules/asure/src/Images/profileImage/UserBase.png`);
    const capa = await read(`${process.cwd()}/node_modules/asure/src/Images/profileImage/UserProfile.png`);
    const mask = await read(`${process.cwd()}/node_modules/asure/src/Images/profileImage/mask.png`);
    const mark = await read(`${process.cwd()}/node_modules/asure/src/Images/profileImage/mark.png`);
    ...
    if(user.displayAvatarURL({ forceStatic: false })?.endsWith(".gif")){
        const nitro = await read(`${process.cwd()}/node_modules/asure/src/Images/profileImage/Badges/v14/NITRO.png`)
        const boost = await read(`${process.cwd()}/node_modules/asure/src/Images/profileImage/Badges/v14/BOOST.png`)
    ...
    if(!user.bot){
        ...
        for (let i = 0; i < flags.length; i++) {
            let badge = await read(`${process.cwd()}/node_modules/asure/src/Images/profileImage/Badges/v14/${flags[i].toUpperCase()}.png`);
            badges.push(badge);
          }
        ...
        if (flags.includes("VerifiedBot")) {
          botBagde = await read(`${process.cwd()}/node_modules/asure/src/Images/profileImage/botVerif.png`);
        } else {
          botBagde = await read(`${process.cwd()}/node_modules/asure/src/Images/profileImage/botNoVerif.png`);
        }
