    ...
    execute: async (interaction, client, args) => {
        ...
        switch(subCommand){
            case "add":
                ...
                const roles = options.getRole("role").id;
            ...
            case "buy":
                ...
                if(buyItem2.role >= guild.me.roles.highest.position) return interaction.reply({embeds: [new MessageEmbed().setTitle("Buy failed").setDescription(`I can't give you this role since it is higher than or equal to my role. Please contact staff and ask them to change it.`).setColor('BLURPLE')]})

                if(member.roles.cache.has(buyItem2.role)) return interaction.reply(`You can't buy this item since you already have this role!`);

                if(buyItem2.role)interaction.guild.members.cache.get(interaction.user.id).roles.add(interaction.guild.roles.cache.get(buyItem2.role));
